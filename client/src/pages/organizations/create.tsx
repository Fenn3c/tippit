...
export default function CreateTipLink() {
    ...
    const formik = useFormik(
        {
            ...
            onSubmit: values => {
                ...
                axiosInstance.post('/api/organizations', formData).then(res => {
                    setStep(4)
                }).catch(err => {
                    console.error(err)
                    setStep(1)
    ...
    return (
        <Layout title="Создание организации">
            <Card bigYpadding className="mb-4">
                ...
                {step === 4 && (
                    <div className="flex flex-col gap-y-8 items-center text-center ">
                        <CompleteIcon />
                        <div>
                            <p className="font-bold text-2xl mb-2">Организация создана!</p>
                            <p className="text-center text-gray-text">Теперь вы можете пригласить сотрудников в организацию</p>
                        </div>
